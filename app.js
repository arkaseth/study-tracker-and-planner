    ...
    exams: [
      {
        ...
        cards: [
          ...
          },
          {
            id: uid(),
            front:
              "Before choosing an answer in RC, what must your evidence do?",
        ...
        mistakes: [
          {
            ...
            topic: "Arithmetic & Algebra",
            question:
              "Ratio problem: mixed up part-to-whole with part-to-part.",
            correct:
              "Set the total as the common denominator before comparing parts.",
...
let state;
try {
  state = JSON.parse(localStorage.getItem(STORAGE_KEY)) || starterData();
} catch {
  state = starterData();
}
if (!state.ai)
...
function save() {
  ...
  if (currentUser) {
    ...
    saveTimeout = setTimeout(() => {
      ...
      supabaseClient
        .from("study_data")
        .upsert({ id: currentUser.id, state: cloudState })
        .then(({ error }) => {
          if (error) {
...
$("#settings-ai-provider").addEventListener("change", (e) => {
  ["gemini", "openai", "claude"].forEach((p) => {
    document
      .querySelector(`[data-provider="${p}"]`)
      .classList.toggle("hidden", e.target.value !== p);
  });
});
...
$("#auth-signup-btn").addEventListener("click", async () => {
  ...
  if (error) {
    ...
    err.textContent =
      "Signup successful! Please check your email and click the confirmation link, then come back here to log in.";
...
function renderTimer() {
  ...
  $("#timer-progress span").style.width =
    `${Math.max(0, Math.min(100, (1 - timerRemaining / total) * 100))}%`;
  $("#timer-start").textContent = timerInterval
    ? "Pause"
    : `Start ${timerMode}`;
  $("#timer-focus").value = state.timer.focus;
  $("#timer-break").value = state.timer.break;
...
function startTimer() {
  ...
  timerInterval = setInterval(() => {
    ...
    if (timerRemaining <= 0) {
      ...
      timerMode = wasFocus ? "break" : "focus";
...
function updateTimerSettings() {
  ...
  state.timer = { focus, break: rest };
  save();
  if (!timerInterval) timerRemaining = timerMinutes() * 60;
  renderTimer();
}
...
function renderHeader() {
  ...
  select.innerHTML = state.exams
    .map(
      (e) =>
        `<option value="${e.id}" ${e.id === state.activeExamId ? "selected" : ""}>${escapeHTML(e.name)}</option>`,
    )
    .join("");
...
function renderDashboard() {
  ...
  $("#today-summary").textContent =
    `${days} days until ${exam.name}. Protect the next helpful session.`;
  $("#stat-grid").innerHTML = [
    ...
    .join("");
  $("#overdue-tasks").innerHTML = overdue.length
  ...
  $("#weekly-total").textContent =
    `${Math.round((weeklyMinutes / 60) * 10) / 10}h logged`;
  $("#due-preview").innerHTML = due.length
    ? due
        .slice(0, 3)
        .map(
          (c) =>
...
function renderAvailabilityEditor(exam) {
  $("#availability-editor").innerHTML = weekdayNames
    ...
    .join("");
  const days = Object.values(exam.availability || {}).filter(
      (d) => d?.active,
    ).length,
    total = availabilityHours(exam);
...
function renderPlan() {
  ...
  $("#plan-capacity").textContent =
    `${availabilityHours(exam)}h available / week`;
  ...
  const horizon = Array.from({ length: 14 }, (_, i) =>
    iso(addDays(new Date(), i)),
  );
  const allDates = [...new Set([...overdueDates, ...horizon])].sort();
  $("#schedule-list").innerHTML = allDates
    .map((d) => {
...
function renderReview() {
  ...
  empty.classList.toggle("hidden", !!due.length);
  stage.classList.toggle("hidden", !due.length);
  if (due.length) {
    const card = due[0];
    $("#card-count").textContent =
      `${due.length} card${due.length === 1 ? "" : "s"} to review`;
    $("#card-front").textContent = card.front;
    $("#card-back").textContent = card.back;
    $("#card-back").classList.add("hidden");
    $("#flashcard .card-label").textContent = "PROMPT · click to reveal";
    $("#review-actions").classList.add("hidden");
    const gaps = calcIntervals(card),
      btn = (r, g) =>
        (document.querySelector(`.rating.${r} small`).textContent =
          g < 1 ? "< 1 day" : `${g} day${g === 1 ? "" : "s"}`);
    btn("again", gaps.again);
    btn("hard", gaps.hard);
    btn("good", gaps.good);
    btn("easy", gaps.easy);
  }
  $("#card-list").innerHTML = exam.cards.length
    ? exam.cards
        .map(
          (c) =>
...
function renderInsights() {
  const e = currentExam(),
    due = getDueCards(e),
    weak = e.topics.filter((t) => t.confidence <= 2),
    totalReview = e.cards.reduce((sum, c) => sum + c.reviews, 0);
...
function savePlanSettings(showToast = false) {
  ...
  const selectedDays = Object.values(availability).filter(
      (d) => d.active,
    ).length,
    average = Math.round((weeklyHours / selectedDays) * 10) / 10;
  save();
  $("#plan-capacity").textContent = `${weeklyHours}h available / week`;
  $("#availability-total").textContent =
    `${weeklyHours}h per week · ${average}h average per selected study day`;
  $("#settings-status").textContent = "Saved automatically.";
...
function applyBulkAvailability() {
  ...
  const selected = [
    ...document.querySelectorAll("[data-availability-day]:checked"),
  ];
  if (!selected.length) {
  ...
  selected.forEach((toggle) => {
    const input = document.querySelector(
      `[data-availability-hours="${toggle.dataset.availabilityDay}"]`,
    );
    input.value = hours;
  });
  savePlanSettings();
...
async function generateSchedule() {
  ...
  exam.tasks = exam.tasks.filter(
    (task) => task.date < today || task.date > end,
  );
  ...
  for (let dayIndex = 0; dayIndex < 14; dayIndex++) {
    ...
    const topicCount =
      dayMinutes <= 120 ? 1 : dayMinutes <= 240 ? 2 : dayMinutes <= 360 ? 3 : 4;
...
function openModal(type, presetDate = iso(), existingTask = null) {
  ...
  const topicOptions = exam.topics
    .map((topic) => `<option value="${escapeHTML(topic.name)}">`)
    .join("");
  if (type === "exam")
    fields = `<label class="modal-field">Plan name<input name="name" placeholder="e.g. CFA Level I" required></label><label class="modal-field">Start from a template<select name="template">${Object.keys(
      templates,
    )
      .map((t) => `<option>${t}</option>`)
      .join(
        "",
      )}</select></label><label class="modal-field">Exam date<input name="examDate" type="date" value="${iso(addDays(new Date(), 90))}" required></label>`;
  if (type === "topic")
    fields =
      '<label class="modal-field">Topic name<input name="topic" placeholder="e.g. Probability" required></label>';
  if (type === "task")
...
function handleModal(form) {
  ...
  if (type === "exam") {
    ...
    state.activeExamId = state.exams.at(-1).id;
...
document.addEventListener("click", (event) => {
  ...
  if (target.closest(".avatar")) {
    $("#mobile-exam-select").innerHTML = state.exams
      .map(
        (e) =>
          `<option value="${e.id}" ${e.id === state.activeExamId ? "selected" : ""}>${escapeHTML(e.name)}</option>`,
      )
  ...
  if (target.id === "settings-delete-keys") {
    appConfirm(
      "Delete API Keys?",
      "This will remove your stored API keys from this browser.",
    ).then((ok) => {
      if (!ok) return;
      state.ai.keys = { gemini: "", openai: "", claude: "" };
      $("#settings-key-gemini").value = "";
      $("#settings-key-openai").value = "";
      $("#settings-key-claude").value = "";
      save();
  ...
  if (target.id === "ocr-button" || target.closest(".ocr-trigger"))
    openOCRModal();
  if (target.id === "ocr-close" || target.id === "ocr-cancel") {
    $("#ocr-dialog").close();
  }
  if (target.id === "ocr-save-mistake") {
    ...
    openModal("mistake");
    setTimeout(() => {
      $('textarea[name="question"]').value = $("#ocr-text-result").value;
    }, 100);
  }
  if (target.id === "ocr-save-flashcard") {
    ...
    openModal("card");
    setTimeout(() => {
      $('input[name="front"]').value = $("#ocr-text-result").value;
    }, 100);
  }
  if (target.dataset.addSessionDate)
    openModal("task", target.dataset.addSessionDate);
  if (target.dataset.editTask) {
    const task = currentExam().tasks.find(
      (item) => item.id === target.dataset.editTask,
    );
    if (task) openModal("task", task.date, task);
  }
  if (target.id === "new-card-button" || target.id === "empty-new-card")
    openModal("card");
  if (target.id === "new-mistake-button") openModal("mistake");
  if (target.id === "apply-bulk-availability") applyBulkAvailability();
  if (target.id === "generate-plan-button") generateSchedule();
  ...
  if (target.id === "flip-card" || target.closest("#flip-card")) {
    ...
    $("#flashcard .card-label").textContent = $(
      "#card-back",
    ).classList.contains("hidden")
      ? "PROMPT · click to reveal"
      : "ANSWER · rate your recall";
  }
  if (target.dataset.rating) {
    ...
    toast(
      r === "again"
        ? "No problem - this card will return today."
        : "Review scheduled.",
    );
  }
  if (target.dataset.deleteTopic) {
    const exam = currentExam();
    exam.topics = exam.topics.filter(
      (topic) => topic.id !== target.dataset.deleteTopic,
    );
    save();
    renderAll();
  }
  if (target.dataset.deleteTask) {
    const exam = currentExam();
    exam.tasks = exam.tasks.filter(
      (task) => task.id !== target.dataset.deleteTask,
    );
    save();
    renderAll();
  }
  ...
  if (target.dataset.deleteMistake) {
    const exam = currentExam(),
      mistake = exam.mistakes.find(
        (item) => item.id === target.dataset.deleteMistake,
      );
    exam.mistakes = exam.mistakes.filter(
      (item) => item.id !== target.dataset.deleteMistake,
    );
  ...
  if (target.dataset.aiCritique) {
    const mistake = currentExam().mistakes.find(
      (m) => m.id === target.dataset.aiCritique,
    );
    if (!mistake) return;
    const btn = target,
  ...
  if (target.dataset.viewConcepts) {
    openConceptsModal(target.dataset.viewConcepts);
  }
  if (target.dataset.deleteConcept) {
    const exam = currentExam(),
      topic = exam.topics.find((t) => t.id === target.dataset.topicId);
    if (!topic) return;
    const concept = topic.concepts.find(
      (c) => c.id === target.dataset.deleteConcept,
    );
    if (concept) {
      if (concept.cardId)
...
function updateTutorialUI() {
  document
    ...
    .forEach((el, i) => el.classList.toggle("hidden", i + 1 !== tutorialStep));
  document
    .querySelectorAll("#tutorial-dots .dot")
    .forEach((el, i) => el.classList.toggle("active", i + 1 === tutorialStep));
...
document.addEventListener("change", (event) => {
  const e = currentExam();
  if (
    event.target.id === "exam-select" ||
    event.target.id === "mobile-exam-select"
  ) {
    state.activeExamId = event.target.value;
    save();
    renderAll();
  ...
  if (event.target.matches("[data-topic-confidence]")) {
    e.topics.find(
      (t) => t.id === event.target.dataset.topicConfidence,
...
$("#plan-settings-form").addEventListener("change", (event) => {
  if (event.target.matches("[data-availability-day]")) {
    const hours = document.querySelector(
      `[data-availability-hours="${event.target.dataset.availabilityDay}"]`,
    );
  ...
  reader.onload = (ev) => {
    try {
      ...
      if (/<script|javascript:|on\w+\s*=|data:/i.test(rawText)) {
        throw new Error(
          "Malicious content detected in backup file. Import blocked for your security.",
        );
      }
      const imported = JSON.parse(rawText);
      if (!imported.activeExamId || !imported.exams)
